...
const int port = 1883;
// デバイスID
char *deviceID = "M5Stack_Door1";  // デバイスIDは機器ごとにユニークにします
// メッセージを知らせるトピック
...
void loop() {
  ...
  if ((curr - prev) >= interval) {  // 前回実行時刻から実行周期以上経過していたら
    ...
    int reading = analogRead(SEC1); //ガスセンサーのアナログ値取得

    doc["DeviceName"] = deviceID;
    doc["data_type"] = "sensor";
    doc["Hg"] = pascals / 3377;
    doc["meters"]   = altm;
    doc["tempC"] = tempC;
    doc["Gas"]   = reading;
  ...
  if (M5.BtnA.wasPressed()) {
    ...
    mqttClient.publish("/sub/M5Stack", "{\"cmd\": \"call_interview\"}");
    playMP3("/ender.mp3", 0.3);
  } else if (M5.BtnB.wasPressed()) {
    M5.Lcd.drawJpgFile(SD, "/home.jpg");
    mqttClient.publish("/sub/M5Stack", "{\"cmd\": \"call_visit\"}");
    playMP3("/BGM02.mp3", 0.3);
  } else if (M5.BtnC.wasPressed()) {
    M5.Lcd.drawJpgFile(SD, "/home.jpg");
    mqttClient.publish("/sub/M5Stack", "{\"cmd\": \"call_other\"}");
